...
func (a Article) Update(db *gorm.DB,values interface{}) error {
	if err := db.Model(&a).Updates(values).Where("id=? AND id_del=?",a.ID).Error;err != nil {
		return err
	}
	return nil
}
...
func (a Article) Get(db *gorm.DB) (Article,error) {
	...
	err := db.First(&article).Error //First用于单条记录的查询，即Limit(1)的SQL语句
...
func (a Article) ListByTagID(db *gorm.DB, tagID uint32, pageOffset, pageSize int) ([]*ArticleRow, error) {
	...
	rows, err := db.Select(fields).Table(ArticleTag{}.TableName()+" AS at"). //Select指定要检索的数据库中的字段
		Joins("LEFT JOIN `"+Tag{}.TableName()+"` AS t ON at.tag_id = t.id"). //Joins指定关联查询的语句
