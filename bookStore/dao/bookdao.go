...
func GetBooks() ([]*model.Book, error) {//返回切片
	//写sql语句
...
func UpdateBook(b *model.Book) error {
	//写sql语句
	sqlStr := "update books set title=?,author=?,price=?,sales=?,stock=? where id=?"//占位符
	//执行
...
func GetPageBooks(pageNo string) (*model.Page, error) {
	...
	*/
	...
	var books []*model.Book//储存当前页图书的切片
	for rows.Next() {
		book := &model.Book{}
		rows.Scan(&book.ID, &book.Title, &book.Author, &book.Price, &book.Sales, &book.Stock, &book.ImgPath)
		//将book添加到books中
